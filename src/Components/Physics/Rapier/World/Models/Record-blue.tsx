...
Command: npx gltfjsx@6.2.16 record-blue.glb -t 
*/
...
export function Model(props: JSX.IntrinsicElements["group"]) {
  const { nodes, materials } = useGLTF(
    `${process.env.PUBLIC_URL}/Models/record-blue.glb`
  ) as GLTFResult;

  const clickHandler = (event: ThreeEvent<MouseEvent>) => {
    window.localStorage.setItem("song", "Danse");
    window.dispatchEvent(new Event("storage"));

    swal("Now playing - Alors on danse - Stromae");
  };
  return (
    <group
      {...props}
...
useGLTF.preload(`${process.env.PUBLIC_URL}/Models/record-blue.glb`);

export { Model as RecordBlue };
