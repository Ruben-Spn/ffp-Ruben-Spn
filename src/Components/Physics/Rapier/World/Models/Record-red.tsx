...
Command: npx gltfjsx@6.2.16 record-red.glb -t 
*/
...
export function Model(props: JSX.IntrinsicElements["group"]) {
  const { nodes, materials } = useGLTF(
    `${process.env.PUBLIC_URL}/Models/record-red.glb`
  ) as GLTFResult;
  const clickHandler = (event: ThreeEvent<MouseEvent>) => {
    window.localStorage.setItem("song", "Everybody");
    window.dispatchEvent(new Event("storage"));

    swal("Now playing - Everybody - Nicki Minaj");
  };
  return (
    <group
      {...props}
...
useGLTF.preload(`${process.env.PUBLIC_URL}/Models/record-red.glb`);

export { Model as RecordRed };
